...
if __name__ == "__main__":
    ...
    # --------------------------------------------#
    # name_classes    = ["background","aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa", "train", "tvmonitor"]
    # name_classes = ["_background_", "farmland", "water", "gobi", "desert", "beararea", ]
    name_classes = ["_background_", "building", "road", "water", "barren", "froest", "agriculture"]
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/test-ccc.txt"), 'r').read().splitlines()
    gt_dir = os.path.join(VOCdevkit_path, "VOC2007/SegmentationClass/")
    miou_out_path = "miou_out"
    pred_dir = os.path.join(miou_out_path, 'detection-results')
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        print("Load model.")
        deeplab = SegFormer_Segmentation(diffusion2, opt)
        print("Load model done.")
